...
class FetchingWorker: NSObject {
   ...
   var usersOutput: UsersDataOutputProtocol!
   var postsOutput: PostsDataBaseObserverDelegate?
   var favoritiesOutput: FavoritesDataBaseObserverDelegate?
   var commentsOutput: CommentsDataBaseObserverDelegate?
   
   let queueUsers = DispatchQueue(label: "com.coded.\(AppConstants.ApplicationName).usersQueue")
   let queuePosts = DispatchQueue(label: "com.coded.\(AppConstants.ApplicationName).postsQueue")
   let queueFavorities = DispatchQueue(label: "com.coded.\(AppConstants.ApplicationName).favoritiesQueue")
...
extension FetchingWorker {
   ...
   fileprivate func fetchAllUsers(completion: @escaping () -> Void) {
   ...
   fileprivate func fetchAllPosts(completion: @escaping () -> Void) {
   ...
   fileprivate func fetchAllFavorities(completion: @escaping () -> Void) {
   ...
   func fetchAllComments(for post: PostModel, addComment: @escaping (CommentItem) -> Void, completion: @escaping () -> Void) {
      guard let postId = post.id else {
         completion()
         return
      }
      let commentsRef = FireBaseManager.firebaseRef.child(CommentItemFields.comments).child(postId)
